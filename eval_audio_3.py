...
from utils import make_weighted_loaders, Loss_3, extract_embeddings_onlyaudio
from model3 import AttentiveAudioEncoder
from train import train_audio_autoencoder_3
from test import test_audio_autoencoder_3
...
def main():
    ...
    epochs = 20
    patience = 4
    save_path = "/home/roano/standalone/models/model3_prova_easy_arcface_5678.pt"
    ...
    # setup model
    encoder = AttentiveAudioEncoder(layers=(5,6,7,8)).to(device)
    #encoder.load_state_dict(torch.load(save_path))
    
    # optimizer and loss
    optimizer = torch.optim.AdamW(encoder.parameters(),lr=lr,weight_decay=1e-2)
    #loss_fn = Loss_3(device,temperature=0.07,sup_weight=1,ce_weight=0.2)
    ...
    for e in range(epochs):
        ...
        train_loss = train_audio_autoencoder_3(encoder,train_dl,optimizer,loss_fn,device)
        train_loss_total.append(train_loss)

        val_loss = test_audio_autoencoder_3(encoder,val_dl,loss_fn,device)
    ...
    test_feats, test_labels,_ = extract_embeddings_onlyaudio(encoder,test_dl,device)
